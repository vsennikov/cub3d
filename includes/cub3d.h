...
typedef struct s_data {
  t_map *map;       // Will point to &game->mappy
  t_player *player; // Will point to &game->player
  void *mlx;        // Will point to game->mlx
  char **parsed_file;
} t_data;
...
void apply_movement(t_player *p, float cos_a, float sin_a, t_game *g);
...
void process_all_map_lines(char **parsed_file, int start_idx, t_data *data,
...
int parse_component_at_index(char **components, int index, t_data *data,
                             int fd);
