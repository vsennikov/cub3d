...
/*   Updated: 2026/01/21 15:35:26 by vsenniko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	process_file_content(t_data *data, int fd)
{
	...
	i = parse_config(data, fd);
	parse_map_data(i, data, fd);
...
int	parse_config(t_data *data, int fd)
{
	...
	i = 0;
	while (data->parsed_file[i] && !all_configs_parsed(data))
	{
		if (ft_strlen(data->parsed_file[i]) > 1)
			parse_config_line(data->parsed_file[i], data, fd);
		i++;
	}
