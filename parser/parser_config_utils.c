...
int	parse_component_at_index(char **components, int index, t_data *data, int fd)
{
	...
	if (!comp_trimmed)
		free_components_and_exit(components, data, fd, "Error: Memory allocation failed\n");
	result = parse_color_component(comp_trimmed, data, fd);
	free(comp_trimmed);
	if (result == -1)
		free_components_and_exit(components, data, fd, "Error: Invalid color format - component out of range\n");
	return (result);
}
