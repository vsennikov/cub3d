...
{
	if (i > 0 && j < ft_strlen(data->map->map[i - 1]) && 
		data->map->map[i - 1][j] == ' ')
		return (1);
	if (i < data->map->height - 1 && j < ft_strlen(data->map->map[i + 1]) && 
		data->map->map[i + 1][j] == ' ')
		return (1);
	if (j > 0 && data->map->map[i][j - 1] == ' ')
		return (1);
	if (j < ft_strlen(data->map->map[i]) - 1 && data->map->map[i][j + 1] == ' ')
		return (1);
	return (0);
}
...
void	process_all_map_lines(char **parsed_file, int s_index, t_data *data, int fd, int map_lines)
{
	int	i;
	int	j;
